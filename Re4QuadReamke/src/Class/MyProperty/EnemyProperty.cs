...
namespace Re4QuadExtremeEditor.src.Class.MyProperty
{
    ...
    public class EnemyProperty : GenericProperty, IInternalID
    {
        ...
        private void EnemyPropertyConstructor(ushort InternalID, UpdateMethods updateMethods, EnemyMethods Methods, bool ForMultiSelection = false)
        {
            ...
            {
                SetThis(this);
                RefreshSpeedScaleVisibility(Methods.ReturnOffset0x00Enable(InternalID));
            }
        ...
        public byte ESL_ENABLE
        {
            ...
            set
            {
                ...
                RefreshSpeedScaleVisibility(value);
                updateMethods.UpdateGL();
            }
        }

        /// <summary>
        /// "Speed Enemy" / "Scale Enemy" only have effect in-game when ENABLE is 0x40 or 0x41 —
        /// in that case they're shown and the raw HX1C-1F bytes they cover are hidden (since
        /// editing them directly would silently conflict with the % values). Otherwise (ENABLE
        /// is anything else, e.g. 0x00 or 0x01) it's the opposite: HX1C-1F show normally and
        /// Speed/Scale are hidden, since the % interpretation doesn't apply. Exactly one of the
        /// two representations is ever visible at a time.
        /// </summary>
        private void RefreshSpeedScaleVisibility(byte enableValue)
        {
            bool showSpeedScale = enableValue == 0x40 || enableValue == 0x41;
            ChangePropertyIsBrowsable(nameof(ESL_SpeedEnemy), showSpeedScale);
            ChangePropertyIsBrowsable(nameof(ESL_ScaleEnemy), showSpeedScale);
            ChangePropertyIsBrowsable(nameof(ESL_HX1C), !showSpeedScale);
            ChangePropertyIsBrowsable(nameof(ESL_HX1D), !showSpeedScale);
            ChangePropertyIsBrowsable(nameof(ESL_HX1E), !showSpeedScale);
            ChangePropertyIsBrowsable(nameof(ESL_HX1F), !showSpeedScale);
        }
        ...
        public ByteObjForListBox ESL_ENABLE_ListBox
        {
            get
            {
                ...
                if (ListBoxProperty.EnemyEnableList.ContainsKey(v))
                {
                    return ListBoxProperty.EnemyEnableList[v];
                }
                else
                {
            ...
            set
            {
                if (value.ID < 0xFF)
                {
                    ...
                    RefreshSpeedScaleVisibility(value.ID);
                    updateMethods.UpdateGL();
                }
            }
        }
        ...
        [CustomCategory(aLang.EnemyCategory)]
        [CustomDisplayName(aLang.ESL_SpeedEnemy_Name)]
        [CustomDescription(aLang.ESL_SpeedEnemy_Description)]
        [TypeConverter(typeof(DecNumberTypeConverter))]
        [DefaultValue(null)]
        [ReadOnly(false)]
        [Browsable(true)]
        [AllowInMultiSelect()]
        [DynamicTypeDescriptor.Id(2600, CategoryID3_Enemy)]
        public ushort ESL_SpeedEnemy
        {
            get => Methods.ReturnSpeedEnemyPercent(InternalID);
            set
            {
                if (value > 999) { value = 999; }
                Methods.SetSpeedEnemyPercent(InternalID, value);
                updateMethods.UpdateGL();
            }
        }

        [CustomCategory(aLang.EnemyCategory)]
        [CustomDisplayName(aLang.ESL_ScaleEnemy_Name)]
        [CustomDescription(aLang.ESL_ScaleEnemy_Description)]
        [TypeConverter(typeof(DecNumberTypeConverter))]
        [DefaultValue(null)]
        [ReadOnly(false)]
        [Browsable(true)]
        [AllowInMultiSelect()]
        [DynamicTypeDescriptor.Id(2700, CategoryID3_Enemy)]
        public ushort ESL_ScaleEnemy
        {
            get => Methods.ReturnScaleEnemyPercent(InternalID);
            set
            {
                if (value > 999) { value = 999; }
                Methods.SetScaleEnemyPercent(InternalID, value);
                updateMethods.UpdateGL();
            }
        }
