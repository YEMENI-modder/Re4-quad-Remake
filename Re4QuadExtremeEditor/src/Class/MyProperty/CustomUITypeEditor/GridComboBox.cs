...
namespace Re4QuadExtremeEditor.src.Class.MyProperty.CustomUITypeEditor
{
    public abstract class GridComboBox : UITypeEditor
    {
		...
		public GridComboBox()
		{
			...
			_listBox.BackColor = Globals.DarkMode ? Color.FromArgb(0x2D, 0x2D, 0x30) : System.Drawing.SystemColors.Control;
		...
		private void myListBox_DrawItem(object sender, DrawItemEventArgs e)
		{
			if (e.Index > -1)
			{
				...
				e.Graphics.DrawString(_listBox.Items[e.Index].ToString(), e.Font, new SolidBrush(textColor), e.Bounds.Left, e.Bounds.Top);
			}
		}
