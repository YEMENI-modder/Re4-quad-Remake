...
namespace Re4QuadExtremeEditor.src.Class.SMX
{
    /// <summary>
    /// Raw 144-byte SMX record layout, copied byte-for-byte from NewAge's SMX/SMX.cs
    /// (RE4_SMX_TOOL.SMX). Field names, offsets and unions (mode 0x00 texture-movement vs
    /// mode 0x01 rotation-speed vs mode 0x02 swing) are preserved exactly as-is, even though
    /// only UseSMXID/Mode/OpacityHierarchy/FaceCulling/AlphaHierarchy/ColorRGB are actually
    /// consumed by the room-rendering path (see SmxEntry.cs / RoomRenderOrder.cs). The rest
    /// is kept for completeness/future use, matching the "no simplification" requirement.
    /// </summary>
    public class SmxRaw
    {
    ...
    /// <summary>
    /// Byte-for-byte port of NewAge's SMX/SMXextract.cs (RE4_SMX_TOOL.SMXextract).
    /// Same magic byte (0x10), same 144-byte record size, same field offsets.
    /// </summary>
    public static class SmxRawExtract
    {
        ...
        public static List<SmxRaw> ToSmx(List<byte[]> lines, bool isPs2)
        {
            List<SmxRaw> res = new List<SmxRaw>();
            for (int i = 0; i < lines.Count; i++)
            {
                SmxRaw smx = new SmxRaw();
                ...
                smx.ColorRGB = new byte[3] { lines[i][0x0C], lines[i][0x0D], lines[i][0x0E] };
