...
namespace Re4QuadExtremeEditor.src.Class.Files
{
    ...
    public class File_LIT_LightEntrys : BaseGroup
    {
        ...
        private byte ReturnByteFromPosition(ushort ID, byte FromPosition)
        {
            return Lines[ID][FromPosition];
        }

        private void SetByteFromPosition(ushort ID, byte FromPosition, byte value)
        {
            Lines[ID][FromPosition] = value;
        }

        private ushort ReturnUshortFromPosition(ushort ID, byte FromPosition)
        {
            return BitConverter.ToUInt16(Lines[ID], FromPosition);
        }

        private void SetUshortFromPosition(ushort ID, byte FromPosition, ushort value)
        {
            byte[] b = BitConverter.GetBytes(value);
            Lines[ID][FromPosition] = b[0];
            Lines[ID][FromPosition + 1] = b[1];
        }
