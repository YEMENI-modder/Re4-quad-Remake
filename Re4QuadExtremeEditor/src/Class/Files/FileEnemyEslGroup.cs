...
namespace Re4QuadExtremeEditor.src.Class.Files
{
    ...
    public class FileEnemyEslGroup
    {
        ...
        public FileEnemyEslGroup()
        {
            ...
            Methods.ReturnSpeedEnemyPercent = ReturnSpeedEnemyPercent;
            Methods.SetSpeedEnemyPercent = SetSpeedEnemyPercent;

            Methods.ReturnScaleEnemyPercent = ReturnScaleEnemyPercent;
            Methods.SetScaleEnemyPercent = SetScaleEnemyPercent;
        ...
        public string GetNodeText(ushort ID)
        {
            ...
            else 
            {
                ...
                {
                    r += "Disable and Speed Activated";
                }
                else if (enable == 0x41)
                {
                    r += "Enable and Speed Activated";
                }
                else
                {
        ...
        /// <summary>
        /// Converts a raw 16-bit value stored at the Speed (0x1C-0x1D) or Scale (0x1E-0x1F) offsets
        /// into the percentage the modder actually wants to see. 0x0000 is a special case meaning 100%;
        /// every other raw value (0-999) is the percentage itself. Values above 255 use both bytes
        /// (little-endian), values 255 and below only use the low byte and leave the high byte at 0.
        /// </summary>
        private static ushort RawEnemyPercentToDisplay(ushort raw)
        {
            return raw == 0 ? (ushort)100 : raw;
        }

        /// <summary>
        /// Converts a percentage (0-999) the modder typed back into the raw value to store.
        /// 100% is stored as the special-case 0x0000; every other value is stored as itself.
        /// </summary>
        private static ushort DisplayEnemyPercentToRaw(ushort percent)
        {
            if (percent > 999) { percent = 999; }
            return percent == 100 ? (ushort)0 : percent;
        }

        private ushort ReturnSpeedEnemyPercent(ushort ID)
        {
            ushort raw = BitConverter.ToUInt16(Lines[ID], 0x1C);
            return RawEnemyPercentToDisplay(raw);
        }

        private void SetSpeedEnemyPercent(ushort ID, ushort percent)
        {
            ushort raw = DisplayEnemyPercentToRaw(percent);
            byte[] b = BitConverter.GetBytes(raw);
            Lines[ID][0x1C] = b[0];
            Lines[ID][0x1D] = b[1];
        }

        private ushort ReturnScaleEnemyPercent(ushort ID)
        {
            ushort raw = BitConverter.ToUInt16(Lines[ID], 0x1E);
            return RawEnemyPercentToDisplay(raw);
        }

        private void SetScaleEnemyPercent(ushort ID, ushort percent)
        {
            ushort raw = DisplayEnemyPercentToRaw(percent);
            byte[] b = BitConverter.GetBytes(raw);
            Lines[ID][0x1E] = b[0];
